...
public class Tabela extends ComponenteMain {
  ...
  protected void addJsCodigo(JavaScriptTag tagJs) {
    ...
    try {
      ...
      // Pesquisa
      strJsCodigo += "$('#";
      strJsCodigo += this.getTxtPesquisa().getStrId();
      strJsCodigo += "').quicksearch('#";
      strJsCodigo += this.getTagTable().getStrId();
      strJsCodigo += " tbody tr');";

      // Ordenação
      strJsCodigo += "$('#";
      strJsCodigo += this.getTagTable().getStrId();
      strJsCodigo += "').tablesorter();";
  ...
  private Tag getTagTable() {
    ...
    try {
      ...
      if (_tagTable == null) {
        ...
        _tagTable.setStrId("tbl_" + this.getTbl().getStrNomeSimplificado());
        ...
        _tagTable.addCss(CssTag.getCssMainInst().setHeight(450, "px"));
